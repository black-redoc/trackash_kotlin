...
class MainActivity : AppCompatActivity(), KodeinAware{
    ...
    private fun bindUI() = runBlocking {
        ...
        val ext = extViewModel.last.await()
        ext.observe(this@MainActivity, Observer {
            if (it == null) return@Observer
            startExtractView(it)
        })
    }

    private fun startExtractView(item: Extract) {
        txtTotal.text = item.total.exchange()
        txtExpenses.text = item.expenses.exchange()
        txtIncomes.text = item.incomes.exchange()
    }
